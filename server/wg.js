...
let wired = {
    ...
    workdir: process.env.WIRED_SERVER_WORKDIR || __dirname, // Рабочая папка
    ...
    reload: {
        yaml() {
            if(wired.yaml === null) {
                try {
                    let configFile = fs.readFileSync(wired.workdir + '../conf/wired.yml', 'utf8');
    ...
    checkHealthy() {
        // yaml-конфиг
        if(!fs.existsSync(wired.workdir + '../conf/wired.yml')) {
            wired.yaml = {
                server: {
                    ip: wired.network,
        ...
        // conf-конфиг
        if(!fs.existsSync(wired.workdir + '../conf/wired.conf')) {
            wired.update.conf();
        }
        // Интерфейс
        ...
        if(checkCmd.status !== 0) {
            ...
            if(spawnSync('wg', ['setconf', wired.interface, 'wired.conf'], {cwd: wired.workdir + '../conf'}).status !== 0)
